...
(function () {
  ...
  function exportPDF() {
    ...
    w.document.write(
      "<!DOCTYPE html><html><head><meta charset=utf-8><title>シフト表</title>" +
        "<style>body{font-family:sans-serif;padding:1rem}table{border-collapse:collapse;width:100%}th,td{border:1px solid #ccc;padding:6px 10px;text-align:left}th{background:#f5f5f5}</style></head><body>" +
        body +
        "</body></html>"
    );
    w.document.close();
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (sess && ALLOWED.indexOf(sess.role) !== -1) {
      enterAdmin(sess);
    } else if (sess) {
    ...
    if (tokBtn)
      tokBtn.addEventListener("click", function () {
        var cur = "";
        try {
          cur = localStorage.getItem("g5_gh_token") || "";
        } catch (e) {}
        var v = prompt(
          "GitHub PAT（repo権限）。空で削除。",
          cur ? "（設定済み・変更する場合は貼付）" : ""
        );
        if (v === null) return;
        if (v === "" || v.indexOf("設定済み") !== -1) {
          if (v === "") {
            try {
              localStorage.removeItem("g5_gh_token");
            } catch (e) {}
            alert("トークンを削除しました");
          }
          return;
        ...
        try {
          localStorage.setItem("g5_gh_token", v.trim());
          alert("トークンを保存しました（このブラウザのみ）");
        } catch (e) {
          alert("保存失敗");
        }
      });

...
(function () {
  ...
  function addToPanel(msg, id, meta) {
    ...
    if (id && !readSet[id]) li.classList.add("notif-unread");
    else if (id) li.classList.add("notif-read");
    ...
    li.addEventListener("click", function () {
      ...
      updateBadge(countUnread() + countLocalNear());
      if (meta && meta.link) location.href = meta.link;
    });
    list.prepend(li);
    ...
    while (attempts < 3) {
      ...
      try {
        if (window.G5Supabase && G5Supabase.insertNotification) {
          ...
          try {
            var base = (window.G5 && G5.BASE) || ".";
            var res = await fetch(base + "/src/data/notifications.json?t=" + Date.now(), { cache: "no-store" });
          ...
          var path = "src/data/notifications.json";
          ...
          if (!putRes.ok) throw new Error(putRes.status + " " + (await putRes.text()).slice(0, 150));
        }
        lastErr = null;
        break;
      } catch (e) {
  ...
  function schedulePoll(failed) {
    ...
    var ms = failed ? POLL_FAIL_MS * Math.min(3, failStreak || 1) : POLL_MS;
    pollTimer = setTimeout(function () {
      pollServerNotifications().finally(function () {
        schedulePoll(false);
      });
    }, ms);
  }
